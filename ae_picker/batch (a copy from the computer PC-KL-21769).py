...
from .pickers import aic_picker, stacked_aic_picker, energy_onset_picker, stalta_picker
...
def pick_file(filepath, signal_type,
              ...
              use_stacked_aic=True,
              use_squared_aic=True,
    ...
    use_stacked_aic : bool
        If ``True`` (default) and *signal_type* is ``'unfiltered'`` with more
        than one channel, average AIC functions across all channels before
        picking.  The single consensus pick is assigned to every channel row.
        Has no effect on filtered signals or single-channel files.
    use_squared_aic : bool
        If ``True`` (default), pass ``use_squared=True`` to ``aic_picker`` /
        ``stacked_aic_picker`` so AIC is computed on ``amp²`` (instantaneous
        energy) rather than raw amplitude.
    plot : bool
